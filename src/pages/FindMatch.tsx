...
import { FaDocker, FaGithub, FaJs, FaNodeJs } from "react-icons/fa";
...
export default function FindMatch() {
  ...
  const handleFindOpponent = () => {
    if (selectedCategory && (username || localStorage.getItem("githubUser"))) {
      setMatchFound(true);
    } else {
      alert(
        ...
        {!matchFound ? (
          ...
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`flex items-center justify-center p-4 border-2 rounded-md transition-colors duration-200 ${
            ...
            <motion.button
              ...
              className={`inline-block text-lg py-4 px-8 rounded-md transition-colors duration-200 ease-in-out ${
                selectedCategory
